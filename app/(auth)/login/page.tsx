      ...
      >
        ...
        {(error || cbError) && (
          <p className="mb-3 text-sm text-red-600">
            {error ||
              (cbError === "CredentialsSignin"
                ? "Invalid credentials"
                : "Sign-in failed")}
          </p>
        )}

        <button className="w-full rounded-lg bg-violet-600 py-2 text-white hover:bg-violet-700">
          Sign in
        </button>

        <p className="mt-4 text-sm text-neutral-600">
          Don&apos;t have an account?{" "}
          <a className="text-violet-700 underline" href="/register">
            Create one
          </a>
        </p>
      </form>
