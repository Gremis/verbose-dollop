  ...
  try {
    ...
    for (const r of rows) {
      ...
      if (r.side === "buy") {
        ...
        // FIX 1: Reduzir totalInvestedUsd proporcionalmente
        ...
        s.totalInvestedUsd -= investedReduced; // ✅ AGORA DIMINUI!

        // FIX 2: Acumular realized profit
        if (Number.isFinite(gainLossUsd)) {
          s.realizedProfitUsd += gainLossUsd; // ✅ AGORA ACUMULA!
        }

        if (s.qtyHeld < 1e-10) {
          s.qtyHeld = 0;
          s.costBasisUsd = 0;
          s.totalInvestedUsd = 0; // ✅ Zera quando vende tudo
        }
    ...
    // FIX 3: Calcular realized profit total somando de todos os assets
    ...
    // FIX 4: Total profit = realized + unrealized
    ...
    return NextResponse.json({
      summary: {
        ...
        totalInvestedUsd,
        profit: {
          realized: { usd: realizedProfitUsd }, // ✅ AGORA CALCULADO!
          unrealized: { usd: unrealizedUsd },
          total: { usd: totalProfitUsd, pct: totalPct }, // ✅ AGORA SOMA OS DOIS!
        },
